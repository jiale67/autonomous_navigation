...
//---------------地面点云过滤-------------------------
  pcl::PointCloud<pcl::PointXYZI>::Ptr laserCloudNoGround(new pcl::PointCloud<pcl::PointXYZI>());
  for (int i = 0; i < laserCloud->points.size(); i++) {
    if (laserCloud->points[i].z >= 0.02) {
      laserCloudNoGround->push_back(laserCloud->points[i]);
    }
  }
  *exploredAreaCloud += *laserCloudNoGround;
//-------------------------------------------------
